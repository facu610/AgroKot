...
class Ubicacion(var activity: AppCompatActivity, ubicacionListener: UbicacionListener) {
    ...
    private fun pedirPermisos(){
        ...
        if(deboProveerContexto){
            ...
            Toast.makeText(activity.applicationContext,"holi", Toast.LENGTH_LONG).show()
            }
        solicitudPermiso()
        ...
        when(requestCode){
            CODIGO_SOLICITUD_UBICACION -> {
                if(grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    ...
                    Toast.makeText(activity.applicationContext,"No diste permiso perro!", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
